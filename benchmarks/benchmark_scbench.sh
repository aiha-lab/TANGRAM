...
GPU_ID=${GPU_ID:-3}
MODEL=${MODEL:-/raid/LLM/Qwen2.5-7B-Instruct-1M}
MAX_LEN=${MAX_LEN:-200000}
EXTRA_ARGS=()

# Multi-GPU example:
#   GPU_ID=0,1,2,3
#   MODEL=/raid/LLM/Qwen2.5-32B
#   MAX_LEN=131072
...
for DATASET in "${DATASETS[@]}"; do
    for RATIO in "${RATIOS[@]}"; do
        ...
        CUDA_VISIBLE_DEVICES="${GPU_ID}" python3 "${SCRIPT_DIR}/benchmark_scbench.py" \
            -d "${DATASET}" \
            --num "${NUM}" \
            --ratio "${RATIO}" \
